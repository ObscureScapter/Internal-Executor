...
local UI = game:GetObjects("rbxassetid://16420766550")[1]
local DragData = {
	Input = nil,
	Dragging = false,
	DragStart = nil,
...
UI.Holder.Size = UDim2.new(0.4, 0, 0.5, 0)
UI.Parent = CoreGui
...
local function BuildLines()
	local ToBuild = UI.Holder.Scroll.Code.Text
	local LineBreaks = ToBuild:split("\n")

	for i = 1, #LineBreaks do
		local NewLine = UI.Holder.Scroll.Code.LineCount:FindFirstChild(i) or UI.Holder.Scroll.Code.LineCount["1"]:Clone()
		NewLine.Name = i
		NewLine.Text = i
		NewLine.Visible = true
		NewLine.Parent = UI.Holder.Scroll.Code.LineCount
	end

	for _,v in UI.Holder.Scroll.Code.LineCount:GetChildren() do
		if v and tonumber(v.Name) and tonumber(v.Name) > #LineBreaks then
			v.Visible = false
		end
	end
...
UI.Holder.Buttons.Clear.MouseButton1Down:Connect(function()
	UI.Holder.Scroll.Code.Text = ""
end)

UI.Holder.Buttons.Execute.MouseButton1Down:Connect(function()
	loadstring(UI.Holder.Scroll.Code.Text)()
end)

local function AdjustText()
	Highlighter.highlight({
		textObject = UI.Holder.Scroll.Code
	})
...
AdjustText()
UI.Holder.Scroll.Code:GetPropertyChangedSignal("Text"):Connect(function()
	AdjustText()
end)

local function Update(input: InputObject)
	local Delta = input.Position - DragData.DragStart
	UI.Holder.Position = UDim2.new(DragData.UIStart.X.Scale, DragData.UIStart.X.Offset + Delta.X, DragData.UIStart.Y.Scale, DragData.UIStart.Y.Offset + Delta.Y)
end

UI.Holder.InputBegan:Connect(function(input: InputObject)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		DragData.Dragging = true
		DragData.DragStart = input.Position
		DragData.UIStart = UI.Holder.Position
...
UI.Holder.InputChanged:Connect(function(input: InputObject)
...
UserInputService.InputChanged:Connect(function(input: InputObject)
	if input == DragData.Input and DragData.Dragging then
		Update(input)
	end
end)
